...
RStudio uses a converter called **Pandoc** after R runs the notebook. If the Render tab first says `output file: filename.knit.md` and then says that the same `.knit.md` file `does not exist`, try this simple reset first:

1. Save your `.Rmd` file and close RStudio.
2. Check that the `.Rmd` file is still in your Chapter folder. If you also see a file ending in `.nb.html`, delete only that file. Windows may show it as `chapter5_notes.nb` and label it **Chrome HTML Document**.
3. Reopen the `.Rmd` file and knit again. RStudio can create a new `.nb.html` file if it needs one.

Do not delete the `.Rmd` file. That is the file containing your work.
