...
RStudio interfaces vary. You may have a 🧶[Knit button]{.glossary-term data-term="Knit Button"}, or you may use the **Preview** drop-down menu.
...
🗣 R displays the definition of the `mean` function object. You do not need to understand every detail yet. For now, notice that typing `mean` without parentheses displays the function instead of calculating a mean.
...
But we haven't yet learned how to change that behavior. This is the moment to use the help page.
...
🦁 **Courage:** Help-page examples may look unfamiliar. You do not need to understand every line before you begin. Look for one pattern that helps you understand how the `trim` argument is used in the `mean()` function.
...
👀 **Cite:** I used `?mean` to read about the `na.rm` argument.

🗣 **Comment:** The help page showed that `na.rm` defaults to `FALSE`. I changed `na.rm` to `TRUE` because my vector contained an `NA` value.
```
```{r}
# ⚡ Code
messy_scores <- c(80, 90, NA, 100)
...
```markdown
🗣 **Comment:** The result is `90` because R averaged `80`, `90`, and `100` after removing `NA` from the calculation.

🪞 **Reflect:** I learned that a default argument can be optional but still important. In this case, `na.rm` had a default value, but I needed to change it for my data.
